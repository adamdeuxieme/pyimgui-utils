...
class TestWindowStackInteractive:
    ...
    def test_window_stack_vertical(self):
        ...
        run_test_window(
            ...
            expected_behavior=["You should see a column of 3 windows, aligned ",
                               "on the the left segment and stacked from top "
                               "to bottom.",
                               "You must observer a slight padding between "
                               "windows",
                               "Moving the first window on the top move every "
                               "stacked windows."]
        )
    ...
    def test_window_stack_horizontal(self):
        ...
        run_test_window(
            window_under_test=wd_ut,
            expected_behavior=["You should see a column of 3 windows, aligned ",
                               "on the the top segment and stacked from left "
                               "to right.",
                               "You must observer a slight padding between "
                               "windows",
                               "Moving the first window on the left move every "
                               "stacked windows."]
        )
